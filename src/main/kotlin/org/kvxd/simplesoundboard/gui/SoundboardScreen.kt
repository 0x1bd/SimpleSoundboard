...
class SoundboardScreen(
    ...
    private var detailInfoText: Text = Text.literal("Select a sound to edit settings").formatted(Formatting.GRAY)
    ...
    fun selectSound(file: File?) {
        ...
        if (file == null) {
            detailInfoText = Text.literal("Select a sound to edit settings").formatted(Formatting.GRAY)
            ...
            detailInfoText = Text.literal("Settings: ${file.name}").formatted(Formatting.YELLOW)
    ...
    private fun updateBindButtonText(keyCode: Int) {
        val keyName =
            if (keyCode > 0)
                InputUtil.fromKeyCode(keyCode, 0).localizedText
            else
                Text.literal("None")
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        val detailsY = height - bottomPaneHeight + 5
        context.drawCenteredTextWithShadow(textRenderer, detailInfoText, width / 2, detailsY, 0xFFFFFF)
    }
    ...
    private inner class ResultListWidget(
        ...
        inner class Entry(private val file: File) : ElementListWidget.Entry<Entry>() {
            ...
            override fun render(
                ...
                val textY = y + (entryHeight - textRenderer.fontHeight) / 2 + 1
                ...
                favBtn.x = x
                favBtn.y = y + (entryHeight - 20) / 2
                favBtn.render(context, mouseX, mouseY, tickProgress)

                playBtn.x = x + entryWidth - playBtn.width
                playBtn.y = y + (entryHeight - 20) / 2
                playBtn.render(context, mouseX, mouseY, tickProgress)
            }
